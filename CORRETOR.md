...
INEP e a metodologia prompt-driven (decomposição por competência,
chain-of-thought, calibração com exemplos reais) validada no TG UFPE.
...
> Este é o arquivo **canônico** da metodologia. Os arquivos em `.claude/skills/`
> e `.cursor/rules/` são gerados a partir deste por `scripts/sync_shims.py`. Edite
> aqui; não edite os shims.
...
4. Se houver viés sistemático (ex: inflar C2), ajustar a leitura da cartilha ou
   de `calibracao_observada.md`. Lembrar: corpus é pré-2025 → divergência
   esperada em C2/C4/C5 não é erro, é drift.
