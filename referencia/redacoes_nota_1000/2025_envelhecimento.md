...
**C2 (200):** Repertório legitimado e **articulado** — Munduruku ("O Karaíba"),
Lei do Sexagenário (história), filme "Vitória" — cada um sustenta uma etapa do
argumento. Dois ângulos contrastantes (histórico × contemporâneo) = domínio do
tema.

**C3 (200):** Projeto claro: tese (envelhecer como privilégio) → desenvolvimento
histórico → contraponto contemporâneo → proposta. Autoria forte na releitura da
Lei do Sexagenário contra o senso comum.

**C4 (200):** Transições variadas ("Nesse contexto", "Historicamente", "Por
outro lado", "Sob essa óptica", "Dessa forma"). Coesão "expressiva" (categoria
2025 — ver `regras_por_ano.md`).

**C5 (200):** Proposta com **agente** (MEC), **ação** (formação/capacitação de
...
- Releitura crítica de um fato ("ao contrário do senso comum") = autoria.
- Proposta 2025 com **ação explícita** + detalhamento por exemplo nomeado.
